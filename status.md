...
### A TRACE BECOMES A REAL SIZE — `profileScaleFromTrace()`, 2026-08-30. **286 of 286.**
The last piece of arithmetic. Traced points are in IMAGE pixels — settled by reading the code,
`v.A.push(canvas2img(cx,cy))`, not assumed — the crop covers a known area of paper, and the
detail was drawn at a known ratio. Compose those and nobody types a dimension:

    a traced 4'-0" x 12'-4" column  ->  1219.2 mm and 3759.4 mm real
                                        at 1:200 the model is 6.10 mm

**`normPoly` CANNOT DO THIS, and that is why the function exists.** It normalises a trace to
its OWN bounding box, which throws away how much of the image the trace spans — and that span
IS the measurement. The extent has to be taken before normalisation. There is a test halving a
trace and requiring the building to halve with it.

### STILL NOT WIRED — and reading the code found the reason it MUST NOT be wired naively
I said this needed somebody watching a trace happen. **That was wrong** — the association is
plain in the source, and looking rather than deferring found a trap.

The association is at the crop-to-view assignment, where the page is in hand:

    for(const bx of pg.boxes){                      // pg IS the page, which carries the record
      const {canvas,cf,topPts,botPts}=cropToViewTrace(bx);
      const view=V[role];
      setRefImage(view,canvas); view.scale=sPxPerMm*cf;

So `view.drawing = pg.drawing` would carry it. One line.

**AND IT WOULD BE SILENTLY WRONG.** `cropToViewTrace(bx)` calls `cropCanvas(bx)`, which crops a
BOX out of the page — so a view's traced points are in the BOX canvas's coordinates, while
`drawing.crop.frame_mm` describes the WHOLE page crop. Calling `profileScaleFromTrace` with the
view canvas size measures the trace as a fraction of the box and then multiplies it by the
page's paper size. **For a box covering a third of the page, the building comes out three times
too small** — plausible, and nothing downstream can tell.

The fix is one of two, and it is a decision rather than an oversight:
- convert the traced points to PAGE coordinates (box offset, then `cf`) and pass the page image
  size, or
- scale `frame_mm` by the box's fraction of the page before applying it.

Either needs the box rect carried alongside the drawing record. **The arithmetic is proven; the
basis is not, and the basis is where the silence lives.** This is the same shape as every other
bug in this file: a whole-file offset, an axis swap, a rotated flip — each invisible to
everything except the one measurement that depends on it absolutely.

### THE DETAIL PICKER — 2026-08-30. **285 of 285.**
Dropping a PDF now lists the details on the sheet, each tappable, with its own scale and how
many of its dimensions were read:

    L406 · 9 details · 1:48 (read off the line work) · 93 dimensions read
      MAIN COLUMN FRONT ELEVATION   · 1:48 · 11 dimensions read
      WAYFINDING SIGN               · 1:32 ·  6 dimensions read
      …

**Split into DATA and DOM on purpose.** `impPdfListItems()` returns what to show and is tested;
`impPdfRenderList()` builds the elements. Two reasons, and both are things this project has
already been bitten by. The first is the INDEX: a list that displays one order and dispatches
another hands somebody a different drawing from the one they tapped — the exact fault fixed a
few days ago between the two endpoints. `index` is the position in `sheet.details` and nothing
sorts or filters between display and dispatch; there is a test walking every entry to say so.

The second is that **a detail title comes out of a PDF**. The first draft built an HTML string
around it and called `escapeHTML()` — which does not exist in this app, because this app puts
untrusted strings in with `textContent` and never builds HTML around them. Returning data keeps
that property instead of reintroducing the problem and then needing a helper to undo it.

### I INVENTED A HELPER AGAIN, ONE TURN AFTER WRITING THAT UP
`escapeHTML()` — same class as `apiBase()` and `backendOn()` last turn, and it parses perfectly.
**The test written last turn to catch exactly this is what caught it**, along with the fact that
`impPdfRenderList` has now moved from invented to real, which failed the ghost list correctly.
Both lists updated. Knowing the failure mode did not stop me making it; the test did.

### A PDF IS NOW READ INSTEAD OF REFUSED — 2026-08-30. **283 of 283.**
The reference-file handler used to turn a PDF away and suggest a manual capture instead. That
is the retyping-by-hand reading a file exists to remove, and a capture discards the scale, the
dimensions and the sheet number along with the vectors.

    impPdfPick()       drops onto the first page that HAS titled details — page 1 of a set is
                       usually a cover or a site plan, and offering those to trace is a dead end
    impPdfSheet()      POST /import/pdf/sheet
    impPdfCrop()       POST /import/pdf/detail
    impPdfUseDetail()  crops one detail and hands it to the tracer as a reference page, keeping
                       the WHOLE crop record — a trace cannot be sized without `axes_swapped`

**There is still no picker UI**, and that is the honest state: these are reachable from the
console and from a drop, but nothing lists the details on screen yet. The list and its layout
want eyes I do not have.

### THREE MISTAKES WORTH KEEPING, all caught by the gate rather than by reading
1. **Three helpers were invented and all parsed perfectly.** `apiBase()`, `backendOn()` and
   `impPdfRenderList()` do not exist — the studio's are `beBase()` and `impAddPage()`. JavaScript
   is happy to call an undefined function until the moment it runs. There is now a test naming
   every helper the import path calls and asserting each is DEFINED, plus that those three
   ghosts are never called.
2. **I used `h()` instead of `t()`.** `h()` registers a WARNING on throw, so broken wiring would
   have read as housekeeping and shipped. Both are `t()` now, and the difference showed up only
   because the test was failing at the time.
3. **A test for the ABSENCE of a string cannot survive a comment describing that string.**
   Checking the source for no "screenshot a page" warned — because the comment explaining what
   the code used to say contains it. Narrowing the regex to `toast(...)` did not help either: a
   regex over a whole file cannot tell a string literal from the prose around it. The check is
   now positional — `impPdfPick` must appear BEFORE the not-an-image rejection, or a drawing is
   turned away before anything reads it.

**And the gate caught my own mess:** `index_mut.html`, left in the scratch directory by a
mutation run, tripped the ship check for duplicate studio copies that will drift. It was never
in the repo, but the check was right to say so.

### THE STUDIO SIDE OF READING A DRAWING — the arithmetic, 2026-08-30
`drawnSpanToReal()` and `profileScaleFromDetail()` in index.html. **281 of 281** (+1
housekeeping warning, pre-existing: "index.html is the only copy of the studio").

The DOM wiring for an import flow is not built — I cannot see a screen, and that part needs
Collin's eyes anyway. What IS built is the part where a mistake is SILENT.

    a 4'-0" column traced across the image  ->  1219.2 mm    exact
    the same trace with the swap ignored    ->    605.8 mm    a believable building, half size

**THE SWAP IS THE WHOLE THING.** The backend returns the crop the right way up, so on a sheet
rotated 270 — which is every sheet in Dylan's set — the image's WIDTH spans the frame's HEIGHT.
Using the frame's own `w` for an image x-span is out by the aspect ratio, 372.3/185.0, very
nearly two. Nothing downstream can tell: the model is plausible and simply wrong. Mutation-
checked by ignoring the swap, which fails the test.

**Plot scale is not the ratio a model is built at**, and the two functions are kept apart so
they cannot be confused: a sheet plotted at 1/4"=1'-0" says nothing about how big a model
anyone wants. `realLength` is carried on the profile rather than recomputed from the model
size, because typing a model length throws the real figure away and nothing can recover it.

**A global flag written and read across two functions was drafted and thrown out** before any
test ran — `vertical_` set immediately before a call, which breaks the moment anything reentres
it. It is a plain parameter. Worth catching in the draft rather than in the field.

### THE CAD TEST IS RUNNABLE HERE NOW — 2026-08-30
`app/cad.py` had **never been exercised outside CI**. The full test is one `generate_bytes`
call of about 290s — 34 lines, nothing to split — so it could not fit a tool call. It still
cannot. But the cost is resolution, and it grows steeply:

    warmup (OpenCascade init)          3.5s, once
    stations 12 arc 10      864 tris   3.0s
    stations 16 arc 12     1178 tris  10.5s     <- the new fast test
    stations 24 arc 16     1372 tris  28.5s
    stations 48 arc 40                 did not finish in 275s   <- the full test, CI only

`test_generate_stl_fast` runs the same path at 16/12 in about 10s and catches an STL path that
is broken outright; the full one still guards resolution-dependent failures in CI. **The STL
path works** — valid binary STL, length matching the triangle count.

**A measurement I nearly published wrong.** The first timing run showed "no wheels" at 39.9s
against "with wheels" at 4.7s, which reads as wheels making it EIGHT TIMES FASTER. It was
OpenCascade initialisation landing in whichever call ran first. After a discarded warmup the
same two are 3.0s and 4.0s. **Time nothing on a kernel's first call.**

### TWO SILENT NON-EXECUTIONS IN THE SAME 34-LINE FILE
1. **`test_generate_stl` reported as a PASS without cadquery.** It caught the ImportError,
   printed a line and `return`ed, so the summary counted it green while nothing had been built.
   Now `pytest.importorskip`, which says what actually happened. Same class as last turn's
   nineteen environment skips reading as passes.
2. **`--deselect ...::test_generate_stl` also deselected `test_generate_stl_fast`**, because
   pytest matches node ids by PREFIX. The suite said "2 deselected" and the fast test quietly
   did not run. The slow one is now `test_generate_stl_full_resolution` so no name prefixes
   another. CI names no tests (`pytest -q`), so the rename is safe.

    118 passed, 1 skipped, 1 deselected  |  schema checker clean

### AFTER A CONTAINER RESET, DO THESE TWO THINGS FIRST. 2026-08-30.
_Nineteen of the twenty "skips" I had been reporting for several turns were environment gaps,
not real ones. **I was shipping schema changes with the schema contract tests silently
skipping.**_

    pip install cadquery --break-system-packages        # plain pip is refused here (PEP 668)
    cd repos && ln -s LEE3D-Lib-main LEE3D-Lib          # and the same for Frontend, Backend-A

The zips extract as `LEE3D-Lib-main`, but the tests look for `LEE3D-Lib` **beside** the backend.
Without the link, four `test_schema_contract.py` tests and two traced-car tests in
`test_hull.py` skip with "check out LEE3D-Lib beside this repo" — and a skip reads as a pass in
the summary line.

    before   98 passed,  20 skipped      <- what I had been reporting
    after   117 passed,   1 skipped      <- the real state

The one remaining skip is legitimate: a clean-error path that can only be exercised WITHOUT
OpenCascade installed.

**This matters beyond tidiness.** The lathe, the backend hollow, the base cut and the schema
contract were all unverified in this container from the reset onward, across every backend
change shipped since. They pass. But "98 passed" was not the reassurance it looked like, and a
skip count is the first thing to read after a reset, not the last.

**`test_cad.py` still cannot be run here.** It collects as ONE test of about 290s, so it cannot
be split or sampled, and the tool limit is ~300s including startup. Two attempts hit the wall at
285s. It remains CI's job, and **it has not been run against the lathe changes in hull.py**
(73d8a585); STATUS's last confirmed green for it is the older 3d50756e.

### THE PDF TESTS ARE BEHAVIOURAL NOW — a real PDF, built in the test. 2026-08-30.
Three checks were source-string assertions: they would have survived a rewrite that kept the
strings and broke the behaviour. PyMuPDF can WRITE a PDF, so `_sheet_pdf()` builds a genuine
one — a framed detail with a title, its own scale, and a dimension of known length — and runs
it at **rotation 0 and 270**, parametrised. Every sheet in the real set is rotated 270, and
that is what hid the y-flip bug.

**IT TOOK THREE ATTEMPTS TO WRITE A TEST THAT CAUGHT IT, and the failures are the lesson:**

    1. crop SIZE against the frame        caught render_detail's flip and the missing
                                          display-space transform. Did NOT catch
                                          extract_geometry's flip: the frame and the crop
                                          shift TOGETHER, and size survives translation.
    2. crop CONTENT (is it blank?)        still did not catch it. An 864pt shift on a
                                          3024pt page left the crop overlapping the drawing,
                                          so there was plenty of ink in the wrong place.
    3. the frame's ABSOLUTE POSITION      caught it.

The fixture draws its frame at PDF points (100, 100, 700, 1500) on an unrotated page 3024 tall,
so in our millimetres y0 must be (3024-1500)*PT_MM = 537.8 and y1 (3024-100)*PT_MM = 1031.9.
Flipping against `page.rect.height` — 2160 on a rotated page — moves both by 304.8mm.

**This is why the bug survived the whole build.** Nothing relative can see a uniform offset:
lengths, spans, proximity, framing, error percentages, even whether a crop contains ink. Only
an absolute check finds it, and until a crop had to be handed back to PDF space there was no
reason to make one. **When a value is only ever used relatively, its absolute correctness is
untested by construction — and it will stay untested until something needs it absolutely.**

All three mutants now fail: render_detail's flip, the missing rotation transform, and
extract_geometry's flip.

    98 passed, 20 skipped  |  schema checker clean

### THE STUDIO CAN NOW REACH ALL OF IT — two endpoints, 2026-08-30
    POST /import/pdf/sheet    page -> the whole sheet.schema.json shape
    POST /import/pdf/detail   page + detail index -> that detail as a PNG, with its own scale

`/sheet` omits bulk geometry unless asked: a real sheet carries 163,000 strokes and a client
that only wants to know what is on the page should not be sent them.

**THE TWO ENDPOINTS INDEXED DIFFERENT LISTS.** `/sheet` published `find_details` output while
`/detail?detail=N` indexed the frame segmentation, so `detail=0` returned an untitled frame
while the listing's first entry was MAIN COLUMN FRONT ELEVATION. **A client picking by index
would have displayed one drawing and traced another**, with nothing to signal it. Each listed
detail now carries the `frame` its geometry lives in, and the crop indexes the list it shows.

**A plan sheet is refused, and the refusal is structural.** L201B and L400 each yield two
frames — the 725 x 951.8mm sheet border and a 61mm seal box — which do not contain one another,
so containment drops neither and both survive untitled. Handing back a whole site plan as
"detail 0" would be worse than a 404. Because the listing carries only TITLED details, such a
page produces an empty list and there is nothing to index: no special case, and none to fail.

    page 7 L406   -> 200  detail 0 = MAIN COLUMN FRONT ELEVATION, 1:48
    page 5 L404   -> 200
    page 0 L201B  -> 404  "no titled details — it is a plan or a schedule"
    page 3 L400   -> 404

    95 passed, 20 skipped  |  schema checker clean

### ONE DETAIL, CROPPED FOR TRACING — `render_detail()`, 2026-08-30
    all 10 framed details on L406 crop at the exact expected size, 0 wrong
    ink rose from ~2% to 4.3-6.1% of sampled pixels once the crop was actually on target

**A LATENT y BUG THAT NOTHING RELATIVE COULD SEE.** Every sheet in this set carries
**rotation 270**. `page.rect` is the ROTATED rectangle — 3024x2160 — while `get_drawings()` and
`get_text()` both report in UNROTATED space, running to 2106x2999, which is the mediabox.
`extract_geometry` flipped y against 2160 instead of 3024, putting **every y out by 864pt —
304.8mm** — with many frames landing at negative coordinates.

**It had been there the whole time and nothing caught it**, because lengths, spans, proximity
and framing are all invariant to a uniform shift. Dimensions linked at 0.00% error throughout.
After the fix every count is identical: 21 / 17 / 92 / 93 dimensions, 10 survey points, same
scales, same details. It surfaced only when a crop had to be handed back to PDF space, where
absolute position is the entire point. **A whole-file offset is invisible to every relative
measurement you can make; only an absolute one finds it.**

**Then a second space, for the same reason.** `get_pixmap(clip=)` wants the ROTATED display
rectangle, not our unrotated millimetres. Without `page.rotation_matrix` the crops clipped
against the wrong page edge — one detail returned 1682px tall where 4397 was right, and it
looked like a perfectly plausible image. Three spaces are in play and all three matter: our mm
(y up, unrotated), PDF points (y down, unrotated), and the display rect (rotated).

**The image comes back the right way up**, so on a rotated sheet its axes are SWAPPED relative
to the frame — a 181x368mm frame renders 4398x2186px. That is what a person should see, so
`rotation`, `axes_swapped` and `frame_mm` are returned beside it rather than left to be
inferred by whoever maps a traced point back to millimetres.

    94 passed, 20 skipped  |  schema checker clean

### A CONSTRUCTION DETAIL IS NOT A PICTURE OF ONE OBJECT. Measured 2026-08-30.
_This changes the shape of the remaining problem, so it is written up rather than worked
around. **Do not spend a session trying to extract a silhouette automatically.**_

Inside one framed detail — MAIN COLUMN FRONT ELEVATION on L406, 15,665 strokes at 1:48:

    74% of strokes are under 0.5mm            11,615 of 15,665, median length 0.085mm
    only 124 strokes are 20mm or longer
    78% of all ink is axis-aligned            49.4% vertical, 28.8% horizontal
    longest stroke 1098.5mm                   the frame itself, bbox 181.0 x 368.3

The tiny strokes are stone-veneer and CMU hatch. **The ink they cover spans 152 x 184mm — but
the column is 4'-0" x 12'-4", which at 1:48 is 25.4 x 78.3mm.** The drawn ink is six times the
area of the object, and no length filter fixes that because the extra ink is not noise.

**The frame's own annotation says why.** Inside it: COMPACTED SUBGRADE, FINISHED GRADE, FOOTING,
and five references out to other drawings. A detail draws the thing IN ITS CONTEXT — the column
with its footing, the ground it stands in, the grade line, the wall running off. Deciding which
of those is "the object" is a judgement the geometry does not carry; only the labels do, and
they point outward ("SEE STRUCTURAL DRAWINGS") as often as they describe.

(An earlier guess that the convention "BEYOND" would mark context did NOT hold here — it appears
0 times on L406, though it is used on L404. Checked rather than assumed.)

### WHAT THIS MEANS, AND IT LANDS ON COLLIN'S OWN RULE
He drew the line himself: **manual input for PICTURES, never for information.** Deciding which
outline is the column is picture work — a human glance settles in a second what no stroke filter
resolves. The scale, the dimensions, the sheet, the placement are information, and those are now
read automatically.

So the target is not an automatic outline. It is a trace step with everything around it already
known:

    before   a 725 x 952mm sheet, 163,293 strokes, scale typed in by hand
    now      ONE 181 x 368mm framed detail, its own scale (1:48) read off the sheet,
             its dimensions already linked and listed, its title known

That is the win, and it is most of the way there. The remaining manual act is the one a person
is better at than any heuristic.

### WHICH STROKES BELONG TO WHICH DETAIL — `detail_frames()` / `segment_by_frame()`
**THE SHEET DRAWS THE ANSWER.** A details sheet frames each detail, and on the real L406 those
frames are an exact grid: 181.0 x 368.3mm cells at x 19.4, 200.4, 381.3, 562.3, inside one
725 x 952mm border.

    L406   10 frames, 9 titled, 9 distinct, 0 details unplaced
           160,438 of 163,293 strokes assigned to a frame
    L404   13 frames, 10 titled, 10 distinct, 0 unplaced
            57,696 of  59,794 strokes assigned

**Two approaches were tried and measured before this one, and both were quietly wrong:**

    nearest-anchor on the matched STROKES   9 regions, 5 of 36 pairs overlapping, two
                                            degenerate to a point, one spanning half the
                                            sheet (a long leader line inflated its box)
    nearest-anchor on the dimension TEXT    9 regions, 3 of 36 overlapping, one claiming
                                            87 dimensions across 260x571mm

Both fail identically: a detail's anchor is its SCALE TEXT, printed at the BOTTOM, so a
dimension at the top of one detail is often nearer the anchor of the detail below it. **A
segmentation that is roughly right is worse than none** — handing a builder two details'
geometry mixed together produces a wrong model in silence.

**PADDING WAS THE ENTIRE FAULT in the first frame attempt.** Frames abut with no gutter:

    pad  0mm -> 9 of 9 details inside exactly one frame
    pad  4mm -> 9 of 9
    pad 12mm -> 3 of 9, with SIX inside several

At 12mm one detail claimed two frames while two others got none. A scale text is printed
INSIDE its own frame; nothing needed allowing for.

The outer border is dropped by CONTAINMENT, not by size — it is the rectangle that holds the
others. Dropping the largest would also drop a legitimately large detail on a bordered-free
sheet. An untitled frame keeps its geometry rather than being discarded: on a real sheet that
is a detail whose title did not parse, and silently dropping it looks like an empty sheet.

**A THIRD FIXTURE THAT PROVED NOTHING.** The padding mutation PASSED at first, because the
fixture put its anchors 20mm from the frame edge — on the real sheet the scale text sits **4.8mm
inside**, which is exactly why 12mm reached the neighbour. Moved to the edge, the mutant fails.
That is three this session: a decoy ranked by the very term under test, a sheet number listed
first so first-seen and largest agreed, and now clearance that made padding harmless.
**A fixture has to reproduce the geometry that CAUSED the bug, not merely the shape of it.**

    92 passed, 20 skipped  |  schema checker clean

### EVERY DETAIL IS NOW MEASURED AT ITS OWN SCALE — 2026-08-30
Finding the per-detail scales was only half of it; `read_sheet` was still measuring the whole
sheet at one ratio. **On the real L406 that silently lost a detail.**

    L406   one sheet scale   87 dimensions
           per detail        93 dimensions, across 9 details
    L404   one sheet scale   92 dimensions
           per detail        92          <- unchanged, as it must be: one scale throughout

The six recovered are the WAYFINDING SIGN, drawn at 3/8"=1'-0" among column sections at 1/4":

    4'-0"   1219.2 mm   drawn 38.10 mm   err 0.00%
    3'-0"    914.4 mm   drawn 28.58 mm   err 0.00%
    2'-10"   863.6 mm   drawn 27.01 mm   err 0.08%

**The failure mode was worse than inaccuracy.** Measured at 1:48 the sign's dimensions are a
third short, the arithmetic check rejects them, and they VANISH. An answer that is wrong
announces itself; an answer that is absent does not. That whole detail was unbuildable from
the sheet it is drawn on, with nothing to say so.

Every match now carries which detail it belongs to, which is the segmentation a caller actually
wants: not "87 dimensions on L406" but "the eleven belonging to MAIN COLUMN FRONT ELEVATION".
`details` is in the sheet contract and the dimension items carry `detail`.

    89 passed, 20 skipped  |  schema checker clean

### WHAT IS ON A SHEET — `find_details()`, 2026-08-30
**A details sheet is not one drawing.** L406 carries nine, and they are not all at one scale.
The title is read back from the scale text, because a detail in this trade always prints its
scale directly beneath its title. Measured on the real L406: title words share one band
position (x 32.6) while the scale sits one line over (x 24.2), 2-6mm of text height against
15-34mm. **The band separates them; the size difference corroborates and is not the test**, as
a title block can print small titles.

    page 2  L301B   1 detail    1:240
    page 4  L403    1 detail    1:96
    page 5  L404   10 details   1:48
    page 6  L405   10 details   1:12
    page 7  L406    9 details   1:48 AND 1:32
    31 titled details across the set

**L406's mixed scales are read correctly, and they explain an earlier result.** Eight column
sections at 1/4"=1'-0" plus `WAYFINDING SIGN` at 3/8". That sheet's refusal to report one
printed scale was never a defect — it genuinely has two.

**Two false-positive classes, both found on the real sheets:**
1. A plan sheet prints its scale beneath a SCALE BAR and a NORTH ARROW. Reading a title back
   from those gave L201B two phantom details, `"N"` and `"SCALE:"`. A label ends in a colon and
   a north arrow is one letter; a real title is neither. Tested against word COUNT rather than
   a two-word rule, because `LOGO` is detail 10 on L404 and a two-word rule loses it.
2. **`"SCALE:"` is itself an anchor** — the parser strips that prefix — and its walk-back
   collected the NEIGHBOURING detail's scale text as a title, yielding a phantom detail called
   `1" = 20'-0"`. Nothing positional prevents that; only asking what the words MEAN does. A
   title that parses as a scale or a dimension is rejected.

**I mistook (2) for a fixture bug first.** The test failed unmutated, the fixture did have both
details sharing one column where the real sheet gives each its own, and fixing that looked like
the answer. It was not — the phantom survived, and chasing it to ground found a real weakness
the corrected fixture still exposed. **A failing test whose fixture is also wrong is two
problems, and fixing the visible one first hides the other.**

**Detail NUMBERS are not captured** and come back None. The bubble is drawn as a circle with
its number elsewhere on the sheet, not adjacent to the title in the text stream. The titles are
unique per sheet, so nothing depends on it yet.

    87 passed, 20 skipped  |  schema checker clean

### A DETAIL BUBBLE NUMBER WAS BECOMING PART OF THE SCALE. Fixed 2026-08-30.
Found by reading the real L406 rather than by a test. A scale text is searched across several
word runs, because a plotted sheet breaks `1/4" = 1'-0"` into separate ones. **That search also
swallowed the detail bubble number printed just before it:**

    bubble 1 + `1/4" = 1'-0"`  ->  read as `1 1/4" = 1'-0"`  ->  1:9.6
    bubble 2                   ->  `2 1/4" = 1'-0"`          ->  1:5.33
    bubble 3                   ->  `3 1/4" = 1'-0"`          ->  1:3.69

Each looks like a scale. None is one. **They polluted the sheet's scale set and made L404 look
ambiguous** — it reported no printed scale at all and fell back on inference.

The fix is a whitelist of ratios someone actually plots at: architectural 3"=1'-0" down to
1/32"=1'-0", engineering 1"=10' through 1"=100', and the metric ratios that share those sheets.
All three false readings are rejected; every real scale is kept. The arithmetic was never
wrong, which is why `standard_only=False` still returns 9.6 and a test pins that — the filter
is doing the work, not a corrected calculation.

**The knock-on is the real prize:**

    before   L404  printed None   inferred 48.0     (ambiguous, guessed)
    after    L404  printed 48.0   inferred None     (read straight off the sheet)

    page 0  L201B  printed 240.0   used 240.0   21 dimensions
    page 4  L403   printed  96.0   used  96.0   17
    page 5  L404   printed  48.0   used  48.0   92
    page 6  L405   printed  12.0   used  12.0    4
    page 7  L406   printed None    used  48.0   87   (genuinely mixed: a 3/8" sign detail
                                                      among the 1/4" ones — inference earns
                                                      its place here)

Also: `1/4" = 1'-0"` was returning **48.00000000000001**. Float noise reaches a caller and an
API response looking like a measurement rather than a ratio. Rounded and snapped.

    84 passed, 20 skipped  |  schema checker clean

### SURVEY POINTS BECOME PLACEABLE — `survey_layout()` / `survey_span()`, 2026-08-30
A schedule gives seven-digit state-plane feet, offset from a datum hundreds of miles away.
Nothing builds from those. A model needs LOCAL millimetres from a chosen origin at the ratio it
is being built to.

**The automated read reproduces the hand reduction exactly.** Before the PDF existed, the
photographed schedule was reduced by hand to 50.84 ft at 104.15 degrees between the monument's
two columns. Parsed and computed:

    column 21 -> column 27:  50.8405 ft, bearing 104.1526 deg from north, 15,496 mm real

    at 1:50   extent 1950.2 x 1079.9 mm   too big for one model
    at 1:100  extent  975.1 x  539.9 mm   too big
    at 1:200  extent  487.5 x  270.0 mm   fits the 5-600mm build range

**That extent spans all TEN points, not the monument.** Points 1 and 2 are wayfinding signs 327
ft away; the monument itself is 50 ft and fits comfortably at 1:50. Filter before reading an
extent as "how big is the thing".

**PLOT SCALE IS NOT MODEL SCALE.** A sheet plotted at 1"=20'-0" is a statement about PAPER. The
model may be built at 1:100 or 1:200 for reasons that have nothing to do with how the drawing
was printed, so the ratio is a PARAMETER and is never taken from the sheet. The signature
exists to make that impossible to get wrong.

**Bearing is clockwise from NORTH, the way a drawing states it** — not the mathematical
counter-clockwise from east. Both conventions produce a plausible number and nothing downstream
would catch the swap; it would simply put the monument across the road. Mutation-checked.

x runs east and y runs north, which is the schedule's own convention. Mapping to a screen is
the caller's business and depends which way the model is laid down.

**The international foot (304.8mm exact) is used.** A state-plane schedule may be in US survey
feet, which differ by 2 parts per million — across this monument's 50ft span that is 0.03mm of
real building, and at any model ratio it is orders of magnitude below what prints.

    82 passed, 20 skipped  |  schema checker clean
